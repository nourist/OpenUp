		...
		<div className="bg-base-100 px-8 md:px-28">
			<header className="fixed top-4 right-8 left-8 flex h-16 items-center gap-1 rounded-xl bg-white/3 p-3 text-sm backdrop-blur-xl md:right-28 md:left-28">
				...
				{user ? (
					...
					<>
						...
						<Link className="bg-primary rounded-full px-4 py-2 text-white capitalize hover:shadow-md" to="/signup">
							{t('sign-up')}
						</Link>
					</>
				)}
			...
			<div className="flex min-h-[100vh] flex-col gap-24 pt-24 md:flex-row">
				<div className="py-20 md:max-w-[min(384px,36%)]">
					...
					<Link to="/signup" className="bg-primary text-base-100 inline-flex gap-2 rounded-full px-8 py-2 font-medium uppercase hover:shadow-md">
						{t('get-started')}
						<ArrowRight />
					</Link>
				</div>
			...
			<div className="mt-12 mb-20 flex flex-col items-center">
				...
				<div className="*:bg-base-content/4 *:hover:bg-primary/80 *:hover:*:text-base-100 grid w-full grid-cols-3 grid-rows-3 gap-3 *:col-span-3 *:space-y-4 *:rounded-xl *:px-[20%] *:py-10 **:transition-all **:duration-200 sm:*:col-span-1 sm:*:row-span-3">
					<div>
			...
			<div className="bg-primary/80 my-18 grid aspect-[5/2] grid-cols-2 gap-20 rounded-4xl px-12 pb-4 *:col-span-2 *:md:col-span-1 lg:px-24">
				<div className="hidden items-center md:flex">
					<ConnectedWorld />
				</div>
				<div className="flex flex-col justify-center gap-8">
					<h1 className="text-2xl font-bold text-white lg:text-5xl xl:text-[4rem]">{t('stay-connected').capitalize()}</h1>
					<div>
						<Link to="/signup" className="text-primary inline-flex gap-2 rounded-md bg-white px-4 py-2 font-semibold hover:shadow-md">
